...
interface CharacterContextualInfoModalProps {
  isOpen: boolean;
  character: AICharacter | null;
  onClose: () => void;
  onSave: (characterId: string, contextualInfo: string) => void;
}

const CharacterContextualInfoModal: React.FC<CharacterContextualInfoModalProps> = ({
  isOpen, character, onClose, onSave
}) => {
  const [infoText, setInfoText] = useState('');
  const textareaRef = useAutoResizeTextarea<HTMLTextAreaElement>(infoText, 250); // Max height 250px

  useEffect(() => {
    if (isOpen && character) {
      setInfoText(character.contextualInfo || '');
    }
  }, [isOpen, character]);
    ...
    >
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full sm:max-w-lg max-h-[90vh] flex flex-col text-gray-200 ring-1 ring-gray-700">
        <div className="flex justify-between items-center mb-4 flex-shrink-0">
          <h2 id="contextual-info-modal-title" className="text-xl font-semibold">Contextual Info for <span className="text-purple-400">{character.name}</span></h2>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-100 rounded-full hover:bg-gray-700" aria-label="Close contextual info editor"><CloseIcon /></button>
        </div>
        ...
        <textarea
          ref={textareaRef}
          placeholder={`Enter contextual prompt for ${character.name}... (e.g., "Describe your current surroundings and mood.")`}
          value={infoText}
          ...
          aria-label={`Contextual information for ${character.name}`}
        />
