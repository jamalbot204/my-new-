...
const AdvancedAudioPlayer: React.FC<AdvancedAudioPlayerProps> = ({
  ...
  const handleRangeMouseUp = () => {
    if (isSeeking && visualSeekTime !== null && totalDuration > 0) {
  ...
  const handleProgressClick = (event: React.MouseEvent<HTMLDivElement>) => {
    if (!event.currentTarget || !totalDuration) return; 
  ...
  const PlayPauseButtonIcon = isLoading
    ? () => (
    ...
    >
      ...
      <div className="flex items-center w-full space-x-1.5 sm:space-x-2">
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 flex-grow justify-center">
            ...
            >
            <PlayPauseButtonIcon className={`w-5 h-5 ${isPlaying && !isLoading ? 'text-orange-400' : (isLoading ? 'text-blue-300' : 'text-green-400')}`} />
            </button>
