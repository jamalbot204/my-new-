...
import { TTS_MODELS, TTS_VOICES, DEFAULT_TTS_SETTINGS, CloseIcon, PencilIcon, MAX_WORDS_PER_TTS_SEGMENT } from '../constants';
import InstructionEditModal from './InstructionEditModal'; // Import the reusable modal

interface TtsSettingsModalProps {
  isOpen: boolean;
  currentSettings: TTSSettings;
  onClose: () => void;
  onApply: (newSettings: TTSSettings) => void;
}

const TtsSettingsModal: React.FC<TtsSettingsModalProps> = ({
  isOpen,
  currentSettings,
  onClose,
  onApply,
}) => {
  const [localTtsSettings, setLocalTtsSettings] = useState<TTSSettings>(currentSettings);
  const [isInstructionModalOpen, setIsInstructionModalOpen] = useState(false);

  useEffect(() => {
    if (isOpen) {
      setLocalTtsSettings({
        ...DEFAULT_TTS_SETTINGS, // Start with defaults
        ...currentSettings,     // Override with current settings
        autoFetchAudioEnabled: currentSettings?.autoFetchAudioEnabled ?? DEFAULT_TTS_SETTINGS.autoFetchAudioEnabled,
        systemInstruction: currentSettings?.systemInstruction || DEFAULT_TTS_SETTINGS.systemInstruction || '',
        maxWordsPerSegment: currentSettings?.maxWordsPerSegment ?? DEFAULT_TTS_SETTINGS.maxWordsPerSegment, 
      });
    }
  ...
  const systemInstructionDisplay = localTtsSettings.systemInstruction || "No system instruction set.";
      ...
      <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex justify-center items-center p-4 backdrop-blur-sm">
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full sm:max-w-md max-h-[90vh] flex flex-col text-gray-200 ring-1 ring-gray-700">
          ...
          <div className="space-y-5 mb-6 overflow-y-auto flex-grow pr-1">
            <div>
              <label htmlFor="tts-model" className="block text-sm font-medium text-gray-300 mb-1">
                TTS Model
              </label>
              <select
                id="tts-model"
            ...
            <div>
              <label htmlFor="tts-max-words" className="block text-sm font-medium text-gray-300 mb-1">
                Max Words Per TTS Segment
              </label>
              <input
                type="number"
                id="tts-max-words"
                ...
                max="1000"
                step="10"
                placeholder={`Default: ${DEFAULT_TTS_SETTINGS.maxWordsPerSegment}`}
              />
            ...
            <div className="border-t border-gray-700 pt-4">
              ...
              >
                <span className={`truncate ${localTtsSettings.systemInstruction ? 'text-gray-200' : 'text-gray-400'}`} title={localTtsSettings.systemInstruction || systemInstructionPlaceholder}>
                  {localTtsSettings.systemInstruction ? (localTtsSettings.systemInstruction.length > 40 ? localTtsSettings.systemInstruction.substring(0, 40) + "..." : localTtsSettings.systemInstruction) : systemInstructionPlaceholder}
                </span>
            ...
            <div className="border-t border-gray-700 pt-4">
              <div className="flex items-center">
                <input
                  id="autoFetchAudioEnabled"
                  name="autoFetchAudioEnabled"
                  type="checkbox"
                  className="h-4 w-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-offset-gray-800"
                  checked={localTtsSettings.autoFetchAudioEnabled ?? false}
                  onChange={handleAutoFetchChange}
                />
                <label htmlFor="autoFetchAudioEnabled" className="ml-2 block text-sm text-gray-300">
