...
import { CloseIcon, TrashIcon, BugAntIcon, ChevronDownIcon, ChevronRightIcon } from '../constants'; // Import Chevrons
import { getModelDisplayName } from '../services/utils'; // Import utility
...
// Removed local ChevronRight and ChevronDown definitions

const LogEntry: React.FC<{ log: ApiRequestLog }> = ({ log }) => {
  const [isExpanded, setIsExpanded] = useState(false);

  // Use getModelDisplayName for consistent model name display
  const modelName = getModelDisplayName(
    typeof log.payload.model === 'string' ? log.payload.model : undefined
  );
...
const DebugTerminalPanel: React.FC<DebugTerminalPanelProps> = ({ isOpen, logs, onClose, onClearLogs, chatTitle }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex justify-center items-center p-4 backdrop-blur-sm">
      <div className="bg-gray-800 p-0 rounded-lg shadow-xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col text-gray-200 ring-1 ring-gray-700">
        ...
        <div className="px-4 pt-2 pb-1">
            <p className="text-xs text-gray-400">Showing logs for chat: <span className="font-medium text-gray-300">{chatTitle}</span></p>
        </div>
