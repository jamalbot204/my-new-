...
import { ChatSession, Attachment, TTSSettings, ExportConfiguration, ChatMessage, ApiRequestLog } from '../types'; // Added ApiRequestLog
import * as dbService from '../services/dbService';
import { METADATA_KEYS } from '../services/dbService'; // Corrected import
import { DEFAULT_SETTINGS, DEFAULT_SAFETY_SETTINGS, INITIAL_MESSAGES_COUNT, DEFAULT_TTS_SETTINGS, DEFAULT_EXPORT_CONFIGURATION } from '../constants';
...
export function useImportExport(
  ...
  showToast: (message: string, type?: 'success' | 'error') => void, // Added showToast
  ...
  const handleExportChats = useCallback(async (chatIdsToExport: string[], exportConfig: ExportConfiguration) => {
    ...
    if(exportConfig.includeLastActiveChatId) {
        ...
        appStateForExport.push({ key: "activeChatId", value: activeId || null });
    }
        ...
        if (file) {
            try {
                ...
                const { 
                    ...
                    displayConfig: importedDispConfig, 
                    activeChatId: importedActiveId,
                    exportConfiguration: importedExportConfig, 
                } = transformImportedData(importedRawData);

                if (importedSessions.length === 0 && !Object.keys(importedGenTimes).length && !importedActiveId && !Object.keys(importedDispConfig).length) {
                ...
                allSessionsAfterImport.forEach(session => {
                    if (importedDispConfig[session.id] !== undefined) {
                        newDisplayConfigFromImport[session.id] = Math.min(session.messages.length, importedDispConfig[session.id]);
                    } else {
                ...
                showToast(`Import successful. ${importedSessions.length} session(s) processed.`, "success");
