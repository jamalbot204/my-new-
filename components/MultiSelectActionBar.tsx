...
import { TrashIcon, AudioResetIcon, XCircleIcon } from './Icons';
...
const MultiSelectActionBar: React.FC = () => {
  ...
  const { handleDeleteMultipleMessages } = chat;
  const { handleResetAudioCacheForMultipleMessages } = audio;

  const selectedCount = selectedMessageIds.size;
  const visibleMessageIds = chat.visibleMessagesForCurrentChat.map(m => m.id);
